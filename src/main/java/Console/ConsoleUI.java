...
public class ConsoleUI {
    ...
    private void createProduct() {
        ...
        /*while (unique < 0) {
            for (Long countFor = 0L; countFor <= count; countFor++) {
                ProductVariables victim = productService.findProductById(count -1);
                if (victim.getName()==product.getName()) {
            ...
            if (price.compareTo(minPrice) < 0) {
                ...
                product.setPercent(percent);
                System.out.println("Price is less than 20 dollars, so percents is 0.");
            }
